...
import { CreatePageDto, UpdatePageDto, NodeRelationDto, CreatePageResult, PageDetailResult, UpdatePageResult, PublishPageResult, PageVersionResult, PageVersionsResult, SharePageResult, DeletePageResult, Page, PageNodeRelation, PageVersion } from './pages.dto';
...
export class PagesService {
  ...
  // 列出用户的所有Pages
  async listPages(user: User, page = 1, pageSize = 20): Promise<{
    pages: Page[];
    total: number;
    page: number;
    pageSize: number;
  ...
  // 创建Page
  async createPage(user: User | ResolveUserResponse, { title = 'Untitled', description, content }: CreatePageDto): Promise<CreatePageResult> {
  ...
  async publishPage(user: User | ResolveUserResponse, pageId: string): Promise<PublishPageResult> {
    ...
    try {
      ...
      content = {
        pageConfig,
        nodes: await Promise.all(nodeRelations.map(async relation => {
          let nodeData = {};
          try {
          ...
          if (relation.node_type === 'code_artifact') {
            try {
              const codeArtifact = await this.codeArtifactService.getCodeArtifactDetail(user, relation.entity_id);
              if (codeArtifact) {
                return {
          ...
          if (relation.node_type === 'knowledge') {
            try {
              const knowledge = await this.knowledgeService.getResourceDetail(user, { resourceId: relation.entity_id });
              if (knowledge) {
                return {
                  relationId: relation.relation_id,
                  nodeId: relation.node_id,
  ...
  async sharePage(user: User | ResolveUserResponse, pageId: string): Promise<SharePageResult> {
    ...
    // 生成唯一分享ID
    const shareId = v5(pageId, v5.URL);
