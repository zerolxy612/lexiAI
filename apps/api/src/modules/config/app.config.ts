...
export default () => {
  // Debug logging for HKGAI credentials
  console.log('🔧 [CONFIG DEBUG] Loading HKGAI credentials:');
  console.log('  - HKGAI_BASE_URL:', process.env.HKGAI_BASE_URL || 'https://dify.hkgai.net');
  console.log(
    '  - HKGAI_RAG_BASE_URL:',
    process.env.HKGAI_RAG_BASE_URL || 'https://ragpipeline.hkgai.asia',
  );
  console.log(
    '  - HKGAI_DIFY_BASE_URL:',
    process.env.HKGAI_DIFY_BASE_URL || 'https://dify.hkgai.net',
  );
  console.log(
    '  - HKGAI_API_KEY:',
  ...
  console.log(
    ...
    process.env.HKGAI_CODE_SEARCH_API_KEY
      ? `${process.env.HKGAI_CODE_SEARCH_API_KEY.substring(0, 10)}...`
      : 'NOT SET',
  );
  ...
  return {
    ...
    credentials: {
      hkgai: {
        ...
        caseSearchKey: process.env.HKGAI_CASE_SEARCH_API_KEY || 'app-Fbs0YwuFNGHlhtAPPlvybrJm',
        codeSearchKey: process.env.HKGAI_CODE_SEARCH_API_KEY || 'app-1rFXyZanlbQJdKtQTaZ3wuSS',
      },
      google: {
        searchApiKey:
