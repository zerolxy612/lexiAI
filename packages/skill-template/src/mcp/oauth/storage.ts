...
export class JsonFileStorage implements IOAuthStorage {
  ...
  private async readStorage(): Promise<OAuthStorageData> {
    ...
    try {
      ...
      throw new Error(
        `Failed to read OAuth storage: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private async writeStorage(data: OAuthStorageData): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to write OAuth storage: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
